...
with col_left:
    ...
    for domain, info in s["domains"].items():
        if not info.get("pass", False):
            lack = info.get("need", 0) - info.get("done", 0)
            st.markdown(f"""
            <div class='card' style='padding:14px 18px;border-left:3px solid {info.get("color", "#7a8099")}'>
    <div style='font-weight:700;font-size:14px'>{domain}</div>
    <div style='font-size:13px;color:#7a8099;line-height:3px'>
        缺 <b style='color:{info.get("color", "#7a8099")}'>{lack} 學分</b> | 已修 {info.get("done", 0)}/{info.get("need", 0)}
        | 已修 {info.get("done", 0)}/{info.get("need", 0)}
              </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    if all(v.get("pass", False) for v in s["domains"].values()) and not miss_req:
