...
<div style='padding:24px 0 16px'>
    <div style='font-size:11px; letter-spacing:3px; color:#4f8ef7; font-weight:700; text-transform:uppercase'>模擬選課規劃</div>
    <h2 style='font-size:26px; font-weight:900; margin:4px 0; color:#FFFFFF'>勾選課程，即時預覽畢業門檻達成狀況</h2>
    <p style='color:#FFFFFF; font-size:14px; opacity:1'>將未來欲修課程打勾後，右側進度即時更新</p>
</div>
""", unsafe_allow_html=True)
...
with col_select:
    ...
    for domain, info in ELECTIVE_DOMAINS.items():
        lack = info.get("need", 0) - s["domains"].get(domain, {}).get("done", 0)
        is_pass = s["domains"].get(domain, {}).get("pass", False)
        badge = "✓ 達標" if is_pass else f"缺 {lack} 學分"
        color = info["color"]

        with st.expander(f"{domain}  {badge}", expanded=not is_pass):
...
with col_preview:
    ...
    for domain, info in sim_s["domains"].items():
        orig = s["domains"].get(domain, {})
        new_done = info.get("done", 0)
        need = info.get("need", 0)
        old_done = orig.get("done", 0)
        color = info.get("color", "#7a8099")
        diff = new_done - old_done
        is_orig_pass = orig.get("pass", False)
        is_info_pass = info.get("pass", False)
        if orig.get("pass", False) and not diff:
            badge = "<span class='badge badge-pass'>達標</span>"
        elif info.get("pass", False):
        ...
        if need > 0:
            ...
            # 如果不需要學分，就直接給 0% 或 100%
            pct_new = 100 if is_info_pass else 0
            pct_old = 100 if is_orig_pass else 0
    ...
    fig = go.Figure(go.Bar(
        ...
        y=[v.get('done', 0) for v in sim_s["domains"].values()],
        marker_color=[v.get('color', '#7a8099') for v in sim_s["domains"].values()],
        text=[f"{v.get('done', 0)}/{v.get('need', 0)}" for v in sim_s["domains"].values()],
    ...
    for i, (domain, info) in enumerate(sim_s["domains"].items()):
        fig.add_shape(type="line", x0=i-0.4, x1=i+0.4,
                      y0=info.get("need", 0), y1=info.get("need", 0),
    ...
    # 最終判斷
    all_domain_pass = all(v.get("pass", False) for v in sim_s["domains"].values())
    ...
    if all_domain_pass and req_all_pass:
        ...
        still_lack = [d for d, v in sim_s["domains"].items() if not v.get("pass", False)]
